...
async def add_user(user: UpdateUser, cursor: psycopg2.extensions.cursor = Depends(connection.get_cursor)):
    try:
        cmd = "INSERT INTO users (login, name, surname, password) VALUES (%s, %s, %s, %s) RETURNING id"
        data = (
            user.login, 
            user.name, 
            user.surname,
...
@router.get("/users/find_by_id")
async def find_user_by_id(id: int, cursor: psycopg2.extensions.cursor = Depends(connection.get_cursor)):
    try:
        redis_info = r.get(id)
        if redis_info:
            return {
                "user": json.loads(redis_info),
                "got_from": "redis cache",
            }

        cmd = f"SELECT id, login, name, surname from users WHERE id = {id}"
        cursor.execute(cmd)
    ...
    return {"user": result, "got_from": "postgres"}
